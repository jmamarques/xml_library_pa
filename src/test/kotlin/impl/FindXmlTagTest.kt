...
internal class FindXmlTagTest{
    ...
    fun findXmlTagTest() {
        ...
        val textFindXmlTag = FindXmlTag { it.name == "text" }
        root.accept(textFindXmlTag)
        assertEquals(true, textFindXmlTag.hasFound)
        assertEquals("text", textFindXmlTag.node?.name)
        assertTrue(textFindXmlTag.node is LeafNode)
        assertEquals("<Zeh>", (textFindXmlTag.node as LeafNode).element)
        // try to find a node do not belong to root
        val nonExistsFindXmlTag = FindXmlTag { it.name == "does not exist" }
